...
export async function handleDevTestInstallation(
    ...
    if (addonDep !== undefined && addonDep.length !== 0) {
        if (addonUUID === '00000000-0000-0000-0000-000000006a91' || addonUUID === '5122dc6d-745b-4f46-bb8e-bd25225d350a') {
            const depObj = {};
        ...
        for (let index = 0; index < addonDep.length; index++) {
            ...
            let addonName = (Object.entries(addonToInstall))[0][0];
            let uuid = ((Object.entries(addonToInstall))[0][1] as any)[0];
...
async function getDependenciesOfAddon(
    service: GeneralService,
    addonUUID,
    varPass,
) {
    const latestVer = (await service.fetchStatus(
