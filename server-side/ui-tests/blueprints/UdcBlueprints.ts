import { Collection, CollectionField, DocumentKey, GridDataView, SchemeField, SchemeFieldType } from "@pepperi-addons/papi-sdk";
...
export class BodyToUpsertUdcWithFields implements Collection {
    ...
    public Type?: 'data' | 'meta_data' | 'indexed_data' | 'index' | 'shared_index' | 'pfs' | 'contained' | 'papi' | 'abstract';
...
export class BaseUdcField implements CollectionField {
    constructor(
        ...
        this.Type = type;
        if (optionalValues?.length) { this.OptionalValues = optionalValues };
        if (items) { this.Items = items };
        if (resource) { this.Resource = resource };
        if (addonUUID) { this.AddonUUID = addonUUID };
        if (indexed) { this.Indexed = indexed };
