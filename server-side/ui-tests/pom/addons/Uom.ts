...
export class Uom extends AddonPage {
    ...
    private async testQtysOfItem(
        ...
        if (aoqmUom1Qty !== undefined)
            expect(await (await this.browser.findElement(workingUomObject.aoqmUom1Qty)).getAttribute('title')).to.be.oneOf[aoqmUom1Qty.toString(), parseFloat(aoqmUom1Qty.toString()).toFixed(2)];
        if (aoqmUom2Qty !== undefined)
            expect(await (await this.browser.findElement(workingUomObject.aoqmUom2Qty)).getAttribute('title')).to.be.oneOf[aoqmUom2Qty.toString(), parseFloat(aoqmUom2Qty.toString()).toFixed(2)];
