...
export async function WorkflowTest(email: string, password: string, client: Client) {
    ...
    describe('Orders UI Tests Suit (New Browser per test (it) scenarios)', async function () {
        ...
        it('Order The Most Expensive Three Items and validate with API', async function () {
            ...
            let webAppSettingsBar = new WebAppSettingsBar(driver);

            await webAppSettingsBar.selectSettingsByID('ERP Integrarion');

            // await webAppSettingsBar.selectSettings("Items");
            debugger;
            await webAppSettingsBar.selectSettingsByID('Data');
            driver.sleep(2000);
            await webAppSettingsBar.selectSettingsByID('Data');
            driver.sleep(2000);
            await webAppSettingsBar.selectSettingsByID('Data');
            debugger;
            driver.sleep(2000);
            webAppSettingsBar = new WebAppSettingsBar(driver);
            await webAppSettingsBar.selectSettingsByID('Data');
            driver.sleep(2000);
            webAppSettingsBar = new WebAppSettingsBar(driver);
            await webAppSettingsBar.selectSettingsByID('Data');
