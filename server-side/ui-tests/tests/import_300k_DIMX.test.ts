...
export async function ImportLimitCSVAdalFromDimx(client: Client, varPass) {
    //
    ...
    const howManyRows_create = 327680; //QTY! -- this is here so we can print it in the log (report)
    ...
    describe('ADAL CREATE SCHEME - IMPORT 327,680 (limit) ROWS USING PFS AND DIMX - EXPORT', async function () {
        ...
        describe('IMPORT 327,680 ROWS USING PFS AND DIMX - EXPORT', async function () {
            it(`TEST IMPORT TO ADAL: RUNNING ON FILE WITH ${howManyRows_create} ROWS, ADAL TABLE NAME: ${schemaName_create}, SCHEME: ${JSON.stringify(
                ...
                console.log(
                    `2. Result Object From Dimx:\n${JSON.stringify(
                        auditLogResponseForImporting1.AuditInfo.ResultObject,
                    )}`,
                );
                console.log(
                ...
                expect(
                    JSON.parse(auditLogResponseForImporting1.AuditInfo.ResultObject).LinesStatistics.Inserted,
                ).to.equal(howManyRows_create);
                //TODO: export
