    ...
    describe('Configuration Dev Tests Suit', async function () {
        ...
        describe('Configuration Addon Dev Tests Runner', () => {
            ...
            it(`1. Get Test Names`, async function () {
                ...
                console.log(
                    `Configuration - ${devTestRunner.addonUUID} Tests, Returned From '/tests' Are: [${testNames}]`,
                );
            });
            it(`2. Run Tests`, async function () {
                for (let index = 0; index < testNames.length; index++) {
                    const testName = testNames[index];
                    console.log(`Running ${testName}, ${index + 1}/${testNames.length}`);
                    const x = await devTestRunner.runDevTest(testName);
                }
            });
        });
    });
