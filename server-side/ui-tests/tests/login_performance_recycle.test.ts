...
export async function LoginPerfTests(email: string, password: string, varPass, client) {
    ...
    describe('Login Performance Tests Suites', () => {
        ...
        describe('Basic UI Tests Suit', async function () {
            ...
            // });
            // for (let index = 1; index < numOfRuns + 1; index++) {${index}/${numOfRuns}
            it(`Loggin With VAR User For The  Time And Reset Nuc For The User About To Be Tested Using VAR UI`, async function () {
                const webAppLoginPage = new WebAppLoginPage(driver);
                ...
                debugger;
                await driver.click(_env === 'prod' ? webAppSettingsSidePanel.VarDistsEditor : webAppSettingsSidePanel.VarDistsBetaEditor);
                ...
                if (_env === 'prod') {
                    ...
                    await varListOfDistsPage.search.enterSearch(email + Key.ENTER, By.xpath("//pep-top-bar//mat-form-field//div//input")); //searching the user to recycle hes nuc
                    expect(await varListOfDistsPage.editPresentedDistNewBeta()).to.be.true; //VAR dist edit page is loaded
                }
