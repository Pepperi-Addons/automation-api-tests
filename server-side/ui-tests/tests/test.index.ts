    ...
    if (tests.includes('Remote_Jenkins_Handler')) {
        ...
        if (addonUUID === 'none') {
            ...
            if (addonUUID === '5122dc6d-745b-4f46-bb8e-bd25225d350a') {
                ...
                const [latestVersionOfAutomationTemplateAddon, entryUUID] =
                    await generalService.getLatestAvailableVersion('02754342-e0b5-4300-b728-a94ea5e0e8f4', varPass);
                ...
                for (let index = 0; index < testsList.length; index++) {
                    ...
                    // debugger;
                    const devTestResutsEu = await getTestResponseFromAuditLog(
                        euUser,
                        'prod',
                        devTestResponseEu.Body.URI,
                    );
                    const devTestResultsSb = await getTestResponseFromAuditLog(
                        sbUser,
                        'stage',
                        devTestResponseSb.Body.URI,
                    );
