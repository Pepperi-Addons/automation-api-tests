...
export async function DataIndexTests(generalService: GeneralService, request, tester: TesterFunctions) {
    ...
    const testData = {
        'server-side': ['00000000-0000-0000-0000-000000040fa9', '1.0.49'],
        'Data Index': ['10979a11-d7f4-41df-8993-f06bfd778304', '0.0.67'],
    };
                ...
                for (let index = 0; index < all_activities_fields.length; index++) {
                    ...
                    describe(allActivitiesFieldName, () => {
                        ...
                        it(`Create Transaction With The New ${allActivitiesFieldName}`, async () => {
                            ...
                            const catalogArr = await objectsService.getCatalogs({ page_size: 1 });
                            catalogInternalID = catalogArr[0].InternalID;
                        ...
                        it(`${allActivitiesFieldName} Total Count Above 0`, async () => {
                            ...
                            if (!updatedSortedAndCountedMap.has(createdField)) {
                                ...
                                throw new Error(`updatedSortedAndCountedMap don't contain the Field: ${createdField}`);
                            }
                        });
                        ...
                        it(`Compare the counts from Totals ${allActivitiesFieldName}`, async () => {
                            ...
                            if (!baseSortedAndCountedMap.has(createdField)) {
                                expect(updatedSortedAndCountedMap).to.be.equal(1);
                            }
                        });
                        ...
                        // 7 (update)	API call POST: https://papi.staging.pepperi.com/V1.0/accounts (With Account.City as the first existing)	Response Code 200
                        ...
                        // 10 (Update to empty)	API call GET:  https://papi.staging.pepperi.com/V1.0/elasticsearch/totals/all_activities?select=count(Account.City)&group_by=Account.City	Count > 0
                        // 11 (Update to empty)	API call POST: https://papi.staging.pepperi.com/V1.0/accounts (With Account.City as empty string or null if possible)	Response Code 200
                        // 12 (Update to empty)	API call GET: https://papi.staging.pepperi.com/V1.0/elasticsearch/totals/all_activities?select=count(Account.City)&group_by=Account.City	Count > 0
                        // 13 (Update to empty)	Compare the counts from Totals Account.City (10), with the counts from Totals Account.City (12)	Is +1 and Is -1
