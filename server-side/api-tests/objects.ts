    ...
    describe('Objects Test Suites', () => {
      ...
      describe('Transactions', () => {
            ...
            it('Create account and TSAs for transactions CRUD', async () => {
                ...
                console.log('The following fields were created:\n' + transactionTSAs);
                transactionLinesTSAs = await service.createBulkTSA('transaction_lines', transactionLineTSAarr, atds[0].TypeID);
            ...
            it('Create transaction', async () => {
                ...
                return Promise.all([
                    ...
                    expect(JSON.stringify(getCreatedTransaction[0].Account)).equals(JSON.stringify({
                        Data: {
                            InternalID: transactionAccount.InternalID,
                    ...
                    expect(getCreatedTransaction[0].ModificationDateTime).to.contain(new Date().toISOString().split('T')[0]),
                    ...
                    expect(getCreatedTransaction[0].TransactionLines).to.include({ URI: '/transaction_lines?where=TransactionInternalID=' + createdTransaction.InternalID })
                ]);
            });
            ...
            it('Create transaction lines', async () => {
                ...
                return Promise.all([
                    ...
                    expect(JSON.stringify(getCreatedTransactionLine[0].Item)).equals(JSON.stringify({
                        Data: {
                    ...
                    expect(JSON.stringify(getCreatedTransactionLine[0].Transaction)).equals(JSON.stringify({
                        Data: {
            ...
            it('Update transaction lines', async () => {
                ...
                expect(updatedTransactionLines = await service.createTransactionLine({
                    TransactionInternalID: createdTransaction.InternalID,
                    LineNumber: 0,
                    ItemExternalID: items[0].ExternalID,
                    UnitsQuantity: 5.0,
                    ...
                    expect(JSON.stringify(updatedTransactionLines.Item)).equals(JSON.stringify({
                        Data: {
                            InternalID: items[0].InternalID,
                            UUID: items[0].UUID,
                            ExternalID: items[0].ExternalID,
                    ...
                    expect(JSON.stringify(updatedTransactionLines.Transaction)).equals(JSON.stringify({
                        Data: {
                            InternalID: createdTransaction.InternalID,
                            UUID: createdTransaction.UUID,
                            ExternalID: createdTransaction.ExternalID,
            ...
            it('Update transaction', async () => {
                expect(updatedTransaction = await service.createTransaction({
                    ExternalID: transactionExternalID,
                    Status: 2,
                    Account: {
                        Data: {
                    ...
                    expect(JSON.stringify(updatedTransaction.Account)).equals(JSON.stringify({
                        Data: {
                            InternalID: transactionAccount.InternalID,
                            UUID: transactionAccount.UUID,
                            ExternalID: transactionAccount.ExternalID,
            ...
            it('Delete transaction test account and TSAs', async () => {
                expect(
                    ...
                    expect(
                        transactionLinesTSAs.length == (await service.deleteBulkTSA('transaction_lines', transactionLineTSAarr, atds[0].TypeID)).length,
                    ).to.be.true,
