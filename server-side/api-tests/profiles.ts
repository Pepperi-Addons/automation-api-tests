    ...
    describe('Profiles Tests Suites', () => {
        ...
        describe('Profiles CRUD', () => {
            ...
            it('Create Profile', async () => {
                ...
                expect(createdProfile).to.have.property('CreationDateTime').that.includes(new Date().toISOString().split('T')[0]);
                ...
                expect(createdProfile).to.have.property('ModificationDateTime').that.includes(new Date().toISOString().split('T')[0]);
            ...
            it('Verify PNS After insert via audit_data_logs', async () => {
                ...
                auditDataLogInsert[0]['ObjectModificationDateTime'] = await profilesService.trimDate(auditDataLogInsert[0]['ObjectModificationDateTime']); //Changing date format due to old bug
                expect(auditDataLogInsert[0]).to.have.property('ObjectKey').that.equals(createdProfile.UUID);
                expect(auditDataLogInsert[0]).to.have.property('ObjectModificationDateTime').that.equals(createdProfile.ModificationDateTime);
            ...
            it('Update Profile', async () => {
                ...
                expect(updatedProfile).to.have.property('InternalID').that.is.a('number').that.equals(createdProfile.InternalID);
                expect(updatedProfile).to.have.property('UUID').that.is.a('string').with.lengthOf(36).that.equals(createdProfile.UUID);
                expect(updatedProfile).to.have.property('CreationDateTime').that.equals(createdProfile.CreationDateTime);
                expect(updatedProfile).to.have.property('Hidden').that.is.false;
                expect(updatedProfile).to.have.property('ModificationDateTime').that.includes(new Date().toISOString().split('T')[0]);
                expect(updatedProfile).to.have.property('ModificationDateTime').that.includes('Z');
                expect(updatedProfile).to.have.property('Name').that.is.equals(profileName + ' Update');
                expect(updatedProfile).to.have.property('ParentInternalID').that.is.equals(repProfile[0].InternalID);
                console.log(`Finished UPDATE profile and verify, name after update: ${updatedProfile.Name}, UUID: ${updatedProfile.UUID}`);
            });
            ...
            it('Verify PNS After update via audit_data_logs', async () => {
                ...
                auditDataLogUpdate[0]['ObjectModificationDateTime'] = await profilesService.trimDate(auditDataLogUpdate[0]['ObjectModificationDateTime']);
                expect(auditDataLogUpdate[0]).to.have.property('ObjectKey').that.equals(createdProfile.UUID);
                expect(auditDataLogUpdate[0]).to.have.property('ObjectModificationDateTime').that.equals(updatedProfile.ModificationDateTime);
                ...
                expect(auditDataLogUpdate[0]).to.have.property('UpdatedFields').that.deep.equals([{
                    FieldID: 'Name',
            ...
            it('Delete Profile', async () => {
                ...
                expect(deletedProfile).to.have.property('InternalID').that.is.a('number').that.equals(createdProfile.InternalID);
                expect(deletedProfile).to.have.property('UUID').that.is.a('string').with.lengthOf(36).that.equals(createdProfile.UUID);
                expect(deletedProfile).to.have.property('CreationDateTime').that.equals(createdProfile.CreationDateTime);
                expect(deletedProfile).to.have.property('Hidden').that.is.true;
                expect(deletedProfile).to.have.property('ModificationDateTime').that.includes(new Date().toISOString().split('T')[0]);
                ...
                console.log(`Finished DELETE profile and verify, name: ${deletedProfile.Name}, UUID: ${deletedProfile.UUID}`);
            ...
            it('Verify PNS After delete via audit_data_logs', async () => {
                ...
                auditDataLogDelete[0]['ObjectModificationDateTime'] = await profilesService.trimDate(auditDataLogDelete[0]['ObjectModificationDateTime']);
                expect(auditDataLogDelete[0]).to.have.property('ObjectKey').that.equals(createdProfile.UUID);
                expect(auditDataLogDelete[0]).to.have.property('ObjectModificationDateTime').that.equals(deletedProfile.ModificationDateTime);
                ...
                expect(auditDataLogDelete[0]).to.have.property('UpdatedFields').that.deep.equals([{
                    FieldID: 'Hidden',
                    NewValue: true,
                    OldValue: false
                }]);
            ...
            it('Insert batch profiles', async () => {
                ...
                for (const key in batchProfiles) {
                    ...
                    expect(batchProfiles[key]).to.have.property('URI').that.equals('/profiles/' + batchProfiles[key].InternalID);
                }
            ...
            it('Update + Delete batch profiles', async () => {
                ...
                for (const key in deleteBatchProfiles) {
                    ...
                    expect(deleteBatchProfiles[key]).to.have.property('URI').that.equals('/profiles/' + deleteBatchProfiles[key].InternalID);
            ...
            it('Verify 100 profiles limit', async () => {
                ...
                const arrayOfProfiles = await profilesService.create110ProfilesArray(profileName, repProfile[0].InternalID);
                ...
                for (const key in batchProfiles) {
                    ...
                    expect(batchProfiles[key]).to.have.property('Message').that.equals(`General error: There are ${startProfilesIncludeDeletedLength} existing profiles and you are trying to add ${arrayOfProfiles.length} and this exeeds the limit of 100`);
                }
                let batchProfilesLength = await profilesService.getProfiles();
                batchProfilesLength = batchProfilesLength.length;
            ...
            it('Verify that parent properties are available', async () => {
                ...
                expect(profileForRepProperties).to.have.property('Name').that.is.equals(profileName + ' parent rep');
                expect(profileForRepProperties).to.have.property('ParentInternalID').that.is.equals(repProfile[0].InternalID);
                expect(profileForAdminProperties).to.have.property('Name').that.is.equals(profileName + ' parent admin');
                expect(profileForAdminProperties).to.have.property('ParentInternalID').that.is.equals(adminProfile[0].InternalID);
                expect(profileForBuyerProperties).to.have.property('Name').that.is.equals(profileName + ' parent buyer');
                expect(profileForBuyerProperties).to.have.property('ParentInternalID').that.is.equals(buyerProfile[0].InternalID);
                const getRepParentFields = await profilesService.getProfiles({
                    where: `Name='${profileForRepProperties.Name}'`,
                    fields: ['Parent.InternalID', 'Parent.UUID', 'Parent.CreationDateTime', 'Parent.Hidden', 'Parent.ModificationDateTime', 'Parent.Name', 'Parent.ParentInternalID']
                });
                getRepParentFields[0]['Parent.CreationDateTime'] = await profilesService.trimDate(getRepParentFields[0]['Parent.CreationDateTime']);          //Changing date format due to old bug
                getRepParentFields[0]['Parent.ModificationDateTime'] = await profilesService.trimDate(getRepParentFields[0]['Parent.ModificationDateTime']);  //Changing date format due to old bug
                expect(getRepParentFields[0]).to.have.property('Parent.InternalID').that.is.a('number').that.equals(repProfile[0].InternalID);
                expect(getRepParentFields[0]).to.have.property('Parent.UUID').that.is.a('string').with.lengthOf(36).that.equals(repProfile[0].UUID);
                expect(getRepParentFields[0]).to.have.property('Parent.CreationDateTime').that.equals(repProfile[0].CreationDateTime);
                expect(getRepParentFields[0]).to.have.property('Parent.Hidden').that.equals(repProfile[0].Hidden);
                expect(getRepParentFields[0]).to.have.property('Parent.ModificationDateTime').that.equals(repProfile[0].ModificationDateTime)
                ...
                getAdminParentFields[0]['Parent.CreationDateTime'] = await profilesService.trimDate(getAdminParentFields[0]['Parent.CreationDateTime']);          //Changing date format due to old bug
                getAdminParentFields[0]['Parent.ModificationDateTime'] = await profilesService.trimDate(getAdminParentFields[0]['Parent.ModificationDateTime']);  //Changing date format due to old bug
                expect(getAdminParentFields[0]).to.have.property('Parent.InternalID').that.is.a('number').that.equals(adminProfile[0].InternalID);
                expect(getAdminParentFields[0]).to.have.property('Parent.UUID').that.is.a('string').with.lengthOf(36).that.equals(adminProfile[0].UUID);
                expect(getAdminParentFields[0]).to.have.property('Parent.CreationDateTime').that.equals(adminProfile[0].CreationDateTime);
                expect(getAdminParentFields[0]).to.have.property('Parent.Hidden').that.equals(adminProfile[0].Hidden);
                expect(getAdminParentFields[0]).to.have.property('Parent.ModificationDateTime').that.equals(adminProfile[0].ModificationDateTime)
                expect(getAdminParentFields[0]).to.have.property('Parent.Name').that.equals(adminProfile[0].Name);
                expect(getAdminParentFields[0]).to.have.property('Parent.ParentInternalID').that.equals(adminProfile[0].InternalID);
                const getBuyerParentFields = await profilesService.getProfiles({
                    where: `Name='${profileForBuyerProperties.Name}'`,
                    fields: ['Parent.InternalID', 'Parent.UUID', 'Parent.CreationDateTime', 'Parent.Hidden', 'Parent.ModificationDateTime', 'Parent.Name', 'Parent.ParentInternalID']
                });
                getBuyerParentFields[0]['Parent.CreationDateTime'] = await profilesService.trimDate(getBuyerParentFields[0]['Parent.CreationDateTime']);           //Changing date format due to old bug
                getBuyerParentFields[0]['Parent.ModificationDateTime'] = await profilesService.trimDate(getBuyerParentFields[0]['Parent.ModificationDateTime']);   //Changing date format due to old bug
                expect(getBuyerParentFields[0]).to.have.property('Parent.InternalID').that.is.a('number').that.equals(buyerProfile[0].InternalID);
                expect(getBuyerParentFields[0]).to.have.property('Parent.UUID').that.is.a('string').with.lengthOf(36).that.equals(buyerProfile[0].UUID);
                expect(getBuyerParentFields[0]).to.have.property('Parent.CreationDateTime').that.equals(buyerProfile[0].CreationDateTime);
                expect(getBuyerParentFields[0]).to.have.property('Parent.Hidden').that.equals(buyerProfile[0].Hidden);
                expect(getBuyerParentFields[0]).to.have.property('Parent.ModificationDateTime').that.equals(buyerProfile[0].ModificationDateTime)
                expect(getBuyerParentFields[0]).to.have.property('Parent.Name').that.equals(buyerProfile[0].Name);
                expect(getBuyerParentFields[0]).to.have.property('Parent.ParentInternalID').that.equals(buyerProfile[0].InternalID);
            ...
            it('Create Profile for user+UI control test', async () => {
                ...
                expect(createdUserUIProfile).to.have.property('CreationDateTime').that.includes(new Date().toISOString().split('T')[0]);
                ...
                expect(createdUserUIProfile).to.have.property('ModificationDateTime').that.includes(new Date().toISOString().split('T')[0]);
                expect(createdUserUIProfile).to.have.property('ModificationDateTime').that.includes('Z');
                expect(createdUserUIProfile).to.have.property('Name').that.is.equals(profileName + ' Parent');
                expect(createdUserUIProfile).to.have.property('ParentInternalID').that.is.equals(repProfile[0].InternalID);
            ...
            it('Create child Profile for user+UI control test', async () => {
                ...
                expect(createdUserUIChildProfile).to.have.property('CreationDateTime').that.includes(new Date().toISOString().split('T')[0]);
                ...
                expect(createdUserUIChildProfile).to.have.property('ModificationDateTime').that.includes(new Date().toISOString().split('T')[0]);
                expect(createdUserUIChildProfile).to.have.property('ModificationDateTime').that.includes('Z');
                expect(createdUserUIChildProfile).to.have.property('Name').that.is.equals(profileName + ' child');
                expect(createdUserUIChildProfile).to.have.property('ParentInternalID').that.is.equals(createdUserUIProfile.InternalID);
            ...
            it('Delete Profile for user+UI control test', async () => {
                ...
                expect(deletedUIProfile).to.have.property('InternalID').that.is.a('number').that.equals(createdUserUIProfile.InternalID);
            ...
            it('Delete child Profile', async () => {
                ...
                expect(deletedChildProfile).to.have.property('InternalID').that.is.a('number').that.equals(createdUserUIChildProfile.InternalID);
