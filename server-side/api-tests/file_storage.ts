...
export async function CRUDOneFileFromFileStorageTest(generalService: GeneralService) {
    ...
    describe('CRUD One File Using The File Storage in Base64', () => {
        ...
        it('Create a file in the file storage', async () => {
            ...
            const allfilesBeforeAddFromBase64: FileStorage[] = await service.getFilesFromStorage();
            ...
            await service.postFilesToStorage(
                service.createNewTextFileFromBase64(testDataFileNameFromBase64, testDataFileNameFromBase64),
            );

            //Get the current (after) files from the File Storage
            await expect((allfilesAfterBase64 = await service.getFilesFromStorage()))
                .to.be.an('array')
                .with.lengthOf(allfilesBeforeAddFromBase64.length + 1);
        });
        ...
        it('Read the new added file content', async () => {
            ...
            const fileContentFromBase64: string = await fetch(uriFromBase64).then((response) => response.text());
        ...
        it('Update the new added file', async () => {
            ...
            const allfilesAfterBase64Update: FileStorage[] = await service.getFilesFromStorage();
            ...
            const updateduriFromBase64 = updatedFileObjectBase64NewUrl.URL;
            updatedFileContentFromBase64 = await fetch(updateduriFromBase64).then((response) => response.text());
        ...
        it('Read the first added file content again after updated the new file', async () => {
            //Get the created file content
            const fileContentFromBase64AfterUpdate: string = await fetch(uriFromBase64).then((response) =>
                response.text(),
            );
        ...
        it('Delete the new file', async () => {
            //Get the current (after) files from the File Storage
            const allfilesAfterBase64Deleted: FileStorage[] = await service.getFilesFromStorage();
    ...
    describe('CRD One File Using The File Storage using URL', () => {
        ...
        it('Create a file in the file storage', async () => {
            //Get the current (before) files from the File Storage
            const allfilesBeforeAddFromURL: FileStorage[] = await service.getFilesFromStorage();

            //Add a file to the File Storage with URL
            testDataFileNameFromURL = 'Test ' + Math.floor(Math.random() * 1000000).toString();
            const testDataFileURL =
                'https://cdn.staging.pepperi.com/30013175/CustomizationFile/9e57eea7-0277-441d-beae-0de365cbdd8b/TestData.txt';
            await service.postFilesToStorage(
                service.createNewTextFileFromUrl(testDataFileNameFromURL, testDataFileNameFromURL, '', testDataFileURL),
            );

            let allfilesAfterURL: FileStorage[];

            //Get the current (after) files from the File Storage
            await expect((allfilesAfterURL = await service.getFilesFromStorage()))
                .to.be.an('array')
                .with.lengthOf(allfilesBeforeAddFromURL.length + 1);
        ...
        it('Read the new added file content', async () => {
            //Get the created file content
            const uriFromURL: string = fileObjectURL.URL as any;
            const fileContentFromURL: string = await fetch(uriFromURL).then((response) => response.text());
        ...
        it('Delete the new file', async () => {
            //Get the current (after) files from the File Storage
            const allfilesAfterURLDeleted: FileStorage[] = await service.getFilesFromStorage();
    ...
    describe('CRD One File Using The File Storage With IsSync = true', () => {
        ...
        it('Create a file in the file storage', async () => {
            //Get the current (before) files from the File Storage
            const allfilesBeforeIsSync: FileStorage[] = await service.getFilesFromStorage();

            //Add a file to the File Storage with URL
            testDataFileNameIsSync = 'Test ' + Math.floor(Math.random() * 1000000).toString();
            const testDataFileIsSync: FileStorage = await service.createNewTextFileFromBase64(
                testDataFileNameIsSync,
                testDataFileNameIsSync,
            );
            ...
            //Get the current (after) files from the File Storage
            await expect((allfilesAfterIsSync = await service.getFilesFromStorage()))
                .to.be.an('array')
                .with.lengthOf(allfilesBeforeIsSync.length + 1);
        });
        ...
        it('Read the new added file properties', async () => {
            ...
            //Get the created file content
            const uriFromIsSync: string = fileObjectIsSync.URL as any;
            fileContentFromIsSync = await fetch(uriFromIsSync).then((response) => response.text());
        ...
        it('Delete the new file', async () => {
            //Get the current (after) files from the File Storage
            const allfilesAfterIsSyncDeleted: FileStorage[] = await service.getFilesFromStorage();
    ...
    describe('Make sure file uploaded via Base64 when using both Base64 and URL', () => {
        ...
        it('Create a file in the file storage', async () => {
            //Get the current (before) files from the File Storage
            const allfilesBeforeAddFromURLAndBase64: FileStorage[] = await service.getFilesFromStorage();

            //Add a file to the File Storage with URL
            testDataFileNameFromURLAndBase64 = 'Test ' + Math.floor(Math.random() * 1000000).toString();
            const testDataFileURLAndBase64 =
                'https://cdn.staging.pepperi.com/30013175/CustomizationFile/9e57eea7-0277-441d-beae-0de365cbdd8b/TestData.txt';
            const testDataFileURLAndBase64Body: FileStorage = await service.createNewTextFileFromBase64(
                testDataFileNameFromURLAndBase64,
                testDataFileNameFromURLAndBase64,
            );
            ...
            //Get the current (after) files from the File Storage
            await expect((allfilesAfterURLAndBase64 = await service.getFilesFromStorage()))
                .to.be.an('array')
                .with.lengthOf(allfilesBeforeAddFromURLAndBase64.length + 1);
        });
        ...
        it('Read the new added file properties', async () => {
            ...
            for (let index = 0; index < allfilesAfterURLAndBase64.length; index++) {
                if (
                    allfilesAfterURLAndBase64[index].FileName?.toString().startsWith(testDataFileNameFromURLAndBase64)
                ) {
        ...
        it('Read the new added file content', async () => {
            //Get the created file content
            const uriFromURLAndBase64: string = fileObjectURLAndBase64.URL as any;
            const fileContentFromURLAndBase64: string = await fetch(uriFromURLAndBase64).then((response) =>
                response.text(),
            );
        ...
        it('Delete the new file', async () => {
            //Get the current (after) files from the File Storage
            const allfilesAfterURLAndBase64Deleted: FileStorage[] = await service.getFilesFromStorage();
            let deletedfileObjectURLAndBase64: any;

            for (let index = 0; index < allfilesAfterURLAndBase64Deleted.length; index++) {
                if (
                    allfilesAfterURLAndBase64Deleted[index].FileName?.toString().startsWith(
                        testDataFileNameFromURLAndBase64,
                    )
                ) {
    ...
    describe('Mandatory Title test (negative)', () => {
        ...
        it('Correct exception message for Title', async () => {
            ...
            const testDataFileNameNonTitle: string = 'Test ' + Math.floor(Math.random() * 1000000).toString();
            const testDataFileNonTitle: FileStorage = await service.createNewTextFileFromBase64(
                testDataFileNameNonTitle,
                testDataFileNameNonTitle,
            );
            const tempBodyNonTitle = {};
            tempBodyNonTitle['Content'] = testDataFileNonTitle.Content;
            tempBodyNonTitle['FileName'] = testDataFileNonTitle.FileName;

            await expect(service.postFilesToStorage(tempBodyNonTitle as any)).to.be.rejectedWith(
                'The mandatory property \\"Title\\" can\'t be ignore.',
            );
        });

        it("Don't Create a file in the file storage", async () => {
            //Get the current (after) files from the File Storage
            await expect(service.getFilesFromStorage())
                .eventually.to.be.an('array')
                .with.lengthOf(letallfilesBeforeAddNonTitle.length);
        });
    });
    ...
    describe('Mandatory FileName test (negative)', () => {
        ...
        it('Correct exception message for FileName', async () => {
            ...
            const testDataFileNameNonFileName = 'Test ' + Math.floor(Math.random() * 1000000).toString();
            const testDataFileNonFileName: FileStorage = await service.createNewTextFileFromBase64(
                testDataFileNameNonFileName,
                testDataFileNameNonFileName,
            );
            const tempBodyNonFileName = {};
            tempBodyNonFileName['Content'] = testDataFileNonFileName.Content;
            tempBodyNonFileName['Title'] = testDataFileNonFileName.Title;

            await expect(service.postFilesToStorage(tempBodyNonFileName as any)).to.be.rejectedWith(
                'The mandatory property \\"FileName\\" can\'t be ignore.',
            );
        });
        ...
        it("Don't Create a file in the file storage", async () => {
            ...
            await expect(service.getFilesFromStorage())
                .eventually.to.be.an('array')
                .with.lengthOf(allfilesBeforeAddNonFileName.length);
        });
    });
    ...
    describe('Mandatory fields test (negative)', () => {
        ...
        it('Correct exception message for FileName', async () => {
            ...
            const testDataFileNameNonMandatory = 'Test ' + Math.floor(Math.random() * 1000000).toString();
            const testDataFileNonMandatory: FileStorage = await service.createNewTextFileFromBase64(
                testDataFileNameNonMandatory,
                testDataFileNameNonMandatory,
            );
            const tempBodyNonMandatory = {};
            tempBodyNonMandatory['Content'] = testDataFileNonMandatory.Content;

            await expect(service.postFilesToStorage(tempBodyNonMandatory as any)).to.be.rejectedWith(
                'The mandatory properties \\"Title\\", \\"FileName\\" can\'t be ignore.',
            );
        });
        ...
        it("Don't Create a file in the file storage", async () => {
            ...
            await expect(service.getFilesFromStorage())
                .eventually.to.be.an('array')
                .with.lengthOf(allfilesBeforeAddNonMandatory.length);
        });
    });
