        ...
        if (whichEnvsToRuns.includes('SB'))
            it('uninstall SB', async () => {
                for (let index = 0; index < SBUsers.length; index++) {
                    ...
                    console.log(`Deleting Addon: ${ADDON_UUID} From User: ${sbUser.email}, User Number: ${index + 1} Out Of ${SBUsers.length} SB Users`);
                    ...
                    if ((auditLogResponseForUninstall as any).Status.Name === 'Failure' && (auditLogResponseForUninstall as any).Status.ID === 0) {
                        expect(auditLogResponseForUninstall.AuditInfo.ErrorMessage).to.include(`Cant uninstall - Addon with UUID ${ADDON_UUID} is not installed for distributor`);
                        ...
                        console.log(`DELETED Addon ${ADDON_UUID}, Version: ${deletedVersion} From User ${sbUser.email}`);
                    }
                }
            });
        if (whichEnvsToRuns.includes('PROD'))
            it('uninstall Prod', async () => {
                for (let index = 0; index < ProdUsers.length; index++) {
                    ...
                    console.log(`Deleting Addon: ${ADDON_UUID} From User: ${prodUser.email}, User Number: ${index + 1} Out Of ${ProdUsers.length} PROD Users`);
                    ...
                    if ((auditLogResponseForUninstall as any).Status.Name === 'Failure' && (auditLogResponseForUninstall as any).Status.ID === 0) {
                        ...
                        expect(auditLogResponseForUninstall.AuditInfo.Type).to.equal('uninstall');
                        const deletedVersion = auditLogResponseForUninstall.AuditInfo.FromVersion;
                        console.log(`DELETED Addon ${ADDON_UUID}, Version: ${deletedVersion} From User ${prodUser.email}`);
                    }
                }
            });
        if (whichEnvsToRuns.includes('EU'))
            it('uninstall EU', async () => {
                for (let index = 0; index < EUUsers.length; index++) {
                    ...
                    console.log(`Deleting Addon: ${ADDON_UUID} From User: ${euUser.email}, User Number: ${index + 1} Out Of ${SBUsers.length} EU Users`);
                    ...
                    if ((auditLogResponseForUninstall as any).Status.Name === 'Failure' && (auditLogResponseForUninstall as any).Status.ID === 0) {
                        ...
                        expect(auditLogResponseForUninstall.AuditInfo.Type).to.equal('uninstall');
                        const deletedVersion = auditLogResponseForUninstall.AuditInfo.FromVersion;
                        console.log(`DELETED Addon ${ADDON_UUID}, Version: ${deletedVersion} From User ${euUser.email}`);
                    }
                }
            });
    });
