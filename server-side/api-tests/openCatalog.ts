...
export async function OpenCatalogTests(generalService: GeneralService, tester: TesterFunctions) {
    ...
    describe('Get single item and compare to PAPI item', () => {
        ...
        it('Verify open catalog single item matches papi item', async () => {
            expect(openCatalogSingleItem).to.have.property('ItemName.Value').that.equals(papiItems[0].Name),
                ...
                expect(openCatalogSingleItem).to.have.property('ItemExternalID.Value').that.equals(papiItems[0].ExternalID),
                ...
                expect(openCatalogSingleItem).to.have.property('Image.Value').that.includes('.pepperi.com/WrntyImages/30014280/PortfolioItems/'),
    ...
    describe('Get items and compare to PAPI items', () => {
        ...
        it('Get items via Open Catalog and papi and compare total count', async () => {
            ...
            expect(papiItems).to.be.an('array');
            openCatalogItems = await openCatalogService.getOpenCatalogItems('?include_count=true&order_by=ItemExternalID.Value');
            expect(openCatalogItems).to.have.property('TotalCount').that.equals(papiItems.length),
                expect(openCatalogItems).to.have.property('Products').that.is.an('array').with.lengthOf(papiItems.length),
                expect(openCatalogItems.Products[0]).to.have.property('ItemName.Value').that.is.a('string').that.equals('Description'),
                ...
                expect(openCatalogItems.Products[0]).to.have.property('ElasticSearchType').that.is.a('string').that.equals('open_catalog'),
        ...
        it('Verify open catalog page size', async () => {
            let pageSizeOpenCatalogData = await openCatalogService.getOpenCatalogItems('?page_size=1');
        ...
        it('Verify open catalog search string', async () => {
            let searchStringOpenCatalogData = await openCatalogService.getOpenCatalogItems('?search_string=TM127');
            expect(searchStringOpenCatalogData).to.have.property('Products').that.is.an('array').with.lengthOf(1),
                expect(searchStringOpenCatalogData.Products[0]).to.have.property('ItemExternalID.Value').that.is.a('string').that.equals('TM127')
        });

        it('Verify open catalog where clause', async () => {
            let whereClauseOpenCatalogData = await openCatalogService.getOpenCatalogItems('?where=ItemExternalID.Value=NE566 and UnitPrice.Value=7.4');
            expect(whereClauseOpenCatalogData).to.have.property('Products').that.is.an('array').with.lengthOf(1),
                expect(whereClauseOpenCatalogData.Products[0]).to.have.property('ItemExternalID.Value').that.is.a('string').that.equals('NE566')
        });
    ...
    describe('Get Filters', () => {
        ...
        it('Get filters and items via Open Catalog and compare distinct values', async () => {
            openCatalogItems = await openCatalogService.getOpenCatalogItems('?fields=ItemExternalID.Value&order_by=ItemExternalID.Value');
            openCatalogFilters = await openCatalogService.getOpenCatalogFilters('?distinct_fields=ItemExternalID.Value');
            const distinctExternalId = Array.from(new Set(openCatalogItems.Products.map((item: any) => item['ItemExternalID.Value'])));
            const distinctExternalIdFilters = Array.from(new Set(openCatalogFilters[0].Values.map((item: any) => item.key)));
    ...
    describe('Get Configurations', () => {
        ...
        it('Get configurations URL and use it to get configurations', async () => {
            ...
            expect(configurationsURL).to.have.property('ConfigurationsURL').that.includes('https://cdn.'),
                expect(configurationsURL).to.have.property('ConfigurationsURL').that.includes('.pepperi.com/30014280/CustomizationFile/'),
                expect(configurationsURL).to.have.property('ConfigurationsURL').that.includes('/configurations.json'),
                configurations = await openCatalogService.getOpenCatalogConfigurations(configurationsURL.ConfigurationsURL);
        ...
        it('Post change to data views, verify change arrived to open catalog after publish', async () => {
            let postDataView = await openCatalogService.postDataView({
                InternalID: 5266676,
                Type: 'Menu',
                Title: 'Rep',
                Context: {
            ...
            let publishOpenCatalog = await openCatalogService.publishOpenCatalog({ atdID: '304550', atdSecret: 'eyJhIjoiMDAwMDAwMDAtMDAwMC0wMDAwLTAwMDAtMDAwMDBjYTdhMTA5IiwidSI6IjY3YjNkY2VlLTU0MmEtNDQyMi04MDFlLWI0ZmVmYjk1ZDYzYiIsImsiOiI0YmMwMWVmZi0zNTc1LTI2N2MtYmU1My0yZTFhNTE3ODg5OWEifQ==', comment: 'test' })
        ...
        it('Revert changed data view to original state and publish', async () => {
            let revertDataView = await openCatalogService.postDataView({
                InternalID: 5266676,
                Type: 'Menu',
                Title: 'Rep',
                Context: {
            ...
            debugger;
            let publishOpenCatalogRevert = await openCatalogService.publishOpenCatalog({ atdID: '304550', atdSecret: 'eyJhIjoiMDAwMDAwMDAtMDAwMC0wMDAwLTAwMDAtMDAwMDBjYTdhMTA5IiwidSI6IjY3YjNkY2VlLTU0MmEtNDQyMi04MDFlLWI0ZmVmYjk1ZDYzYiIsImsiOiI0YmMwMWVmZi0zNTc1LTI2N2MtYmU1My0yZTFhNTE3ODg5OWEifQ==', comment: 'test' })
