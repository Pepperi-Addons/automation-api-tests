    ...
    describe('UDC Tests Suites', () => {
        ...
        describe('Base Collection Testing', () => {
            ...
            it('Positive Test: DIMX overwrite test', async () => {
                ...
                const auditLogDevTestResponse = await generalService.getAuditLogResultObjectIfValid(executionURI as string, 120, 7000);
                ...
                const howManyOld = 5;
                const howManynew = 5;
                await createUpdatedData(howManyOld, howManynew);
                ...
                const bodyToSendOverWrite = { URI: tempFileNewResponse.TemporaryFileURL, Overwrite: true };
                ...
                const overwriteResponse = await generalService.getAuditLogResultObjectIfValid(executionURIOverWrite as string, 120, 7000);
                debugger;
                ...
                expect(overwriteLineStats.Total).to.equal(howManyOld+howManynew);
            });
            it("Tear Down: cleaning all upserted UDC's", async () => {
                ...
                const toHideCollections = documents.filter(
                    (doc) =>
                        ...
                        doc.Name.includes('DimxOverwrite'),
                );
...
async function createUpdatedData(howManyOldRows: number, howMantNewRows: number) {
    const headers = "code";
    const runningDataCode = "data_index";
    const runningDataCodeNew = "new_index";
    let strData = "";
    strData += headers + "\n";
    for (let index = 0; index < howManyOldRows; index++) {
        strData += `${runningDataCode.replace('index', index.toString())}\n`;
    }
    for (let index = 0; index < howMantNewRows; index++) {
