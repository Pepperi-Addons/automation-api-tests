...
export async function ExecuteSyncTests(generalService: GeneralService, tester: TesterFunctions) {
        ...
        describe('Endpoints And Data Members Validations', () => {
            ...
            if (isBase) {
                ...
                it('Simple Sync Put Data Members Validation', async () => {
                    const testBody = {} as SyncBody;
                    Object.assign(testBody, _body);
                    testBody.LocalDataUpdates = Object.assign({}, _localData);
    ...
    async function cleanUpTest() {
        ...
        //three months back from today date creation:
        const d = new Date();
        d.setMonth(d.getMonth() - 3);
        const dateSplittedWithZeros = d
            ...
            .split('/');
        const dateFormmated = `${dateSplittedWithZeros[2]}-${dateSplittedWithZeros[1]}-${dateSplittedWithZeros[0]}`;
        const allActivitiesArr: GeneralActivity | Transaction = await service.papiClient.allActivities.find({
            where: `CreationDateTime>'${dateFormmated}Z'`, //EVGENY - 18/4/24: Changed date from hard coded date to 3 months back to make the test more stable
            page_size: -1,
    ...
    async function waitForSyncStatus(uuid: string, maxTime: number) {
        ...
        do {
            ...
            (apiGetResponse.Status == ('SyncStart' as SyncStatus) ||
                apiGetResponse.Status == ('New' as SyncStatus) ||
                apiGetResponse.Status == ('GetInProgress' as SyncStatus)) &&
