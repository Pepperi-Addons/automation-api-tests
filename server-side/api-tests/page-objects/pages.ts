...
export async function PagesTestSuite(generalService: GeneralService, tester: TesterFunctions) {
	...
	const testData = {
		...
		'sub-addon-2': ['f93658be-17b6-4c92-9df3-4e6c7151e038', ''], //Slideshow Addon 0.0.36
	};
	...
	describe('Pages API Tests Suite', function () {
		...
		describe('Load Testing', function () {
			...
			// it(`Create page with ${objectsToCreate} Page Blocks and Sections`, async function() {
			//     const tempPage = PageFactory.defaultPage();
			//     tempPage.Name = "Huge Page Test";
			//     const testPage: PageClass = new PageClass(tempPage);

			//     for (let i = 0; i < objectsToCreate; i++) {
			//         const pageSection : PageSection = PageFactory.defaultSection();
			//         testPage.addSection(pageSection);
			//         const pageBlock : PageBlock = PageFactory.defaultPageBlock(pageBlockRelation);
			//         testPage.addNewBlockToSection(pageBlock, pageSection.Key, 0);
			//     }
			//     const result = await pagesService.createOrUpdatePage(testPage.page);
			//     pagesService.deepCompareObjects(testPage.page, result, expect);
			// });
