...
export async function DBSchemaTestsPart2(generalService: GeneralService, request, tester: TesterFunctions) {
    ...
    describe('ADAL Tests Suites', () => {
        ...
        describe('Create Schema_Part2', () => {
            ...
            it('Negative - Try to create schema with UserDefine true with PascalCase on Field name', () => {
                assert(logcash.createSchemaUDFalseFieldsNegativeStatus,  logcash.createSchemaUDFalseFieldsNegativeErrorMessage);
            });
            it('Negative - Try to create schema with UserDefine true with special character [!] on schema name', () => {
                assert(logcash.createSchemaNegativePatternSpecialCharOnNameStatus, logcash.createSchemaNegativePatternSpecialCharOnNameErrorMessage );
            });
        ...
        describe('Contaned table type verification', () => {
            it('Create schema with type contained', async () => {
                assert(
                    logcash.createContainedSchemaPositiveStatus,
            ...
            it('Negative - try to insert data to contained table', () => {
                assert(logcash.insertDataToContainedTableNegativeStatus, logcash.insertDataToContainedTableNegativeError);
            });
        ...
        describe('Hide/unhide functionality test', () => {
            it('Create schema with type data', async () => {
                assert(
                    logcash.createSchemeStatus,
                    logcash.createSchemeErrorMessage,
                );
            });
    ...
    async function createSchemaUDFalseNegative() {    //user define is default = false
        ...
        //debugger;
        if (
            logcash.createSchemaUDFalseNegative.fault.faultstring.includes("Invalid schema name, the name 'negative-camelCase1' did not match the pattern")
        ) {
    ...
    async function createSchemaUDFalseFieldsNegative() {    //user define is default = false
        ...
        //debugger;
        if (
            logcash.createSchemaUDFalseFieldsNegative.fault.faultstring.includes("Invalid field name, the field name 'TestStringPascalCase' did not match the pattern")
        ) {
    ...
    async function createSchemaNegativePatternSpaceOnName() {    //user define is default = false
        ...
        //debugger;
        if (
            logcash.createSchemaNegativePatternSpaceOnName.fault.faultstring.includes("Failed due to exception: Invalid schema name, the name 'PascalCase test' did not match the pattern ^[A-Z][-a-zA-Z_0-9]*$")
        ) {
    ...
    async function createSchemaNegativePatternSpecialCharOnName() {    //user define is default = false
        ...
        //debugger;
        if (
            logcash.createSchemaNegativePatternSpecialCharOnName.fault.faultstring.includes("Invalid schema name, the name 'PascalCase!test' did not match the pattern ^[A-Z][-a-zA-Z_0-9]*$")
        ) {
...
async function createSchemaPositive() {    //user define is default = false
...
async function updateSchemaNegative() {    //user define is default = false
    ...
    //debugger;
    if (
        logcash.updateSchemaNegative.fault.faultstring.includes("Failed due to exception: It's not possible to change UserDefined value after schema creation")
    ) {
        logcash.updateSchemaNegativeStatus = true;
    } else {
        logcash.updateSchemaNegativeStatus = false;
        logcash.updateSchemaNegativeErrorMessage =
            'UserDefined proproperty can only be set on creation'
    }
    await createExtendingSchemePositive();
}
//#endregion
//#region Extending Schemes
async function createExtendingSchemePositive() {    //user define is default = false
...
async function updateExtendedSchema() {    //user define is default = false
    ...
    //debugger;
    if (
        logcash.updateExtendedSchema.Fields.testString1_updated.Type == 'String'
    ) {
        logcash.updateExtendedSchemaStatus = true;
    } else {
        logcash.updateExtendedSchemaStatus = false;
        logcash.updateExtendedSchemaErrorMessage =
            'Schema will be updated successfully, but actually failed'
    }
    await updateExtendingScheme();
}

async function updateExtendingScheme() {    //user define is default = false
...
async function updateExtendingSchemeNegative() {    //user define is default = false
    ...
    //debugger;
    if (
        logcash.updateExtendingSchemeNegative.fault.faultstring.includes(" Cannot extend schema, fields [testString1_updated] exist in both schemas")
    ) {
...
async function dropExtendingTable() {
    ...
    if (res.Ok) {
        ...
        logcash.dropExtendingTableError =
            'Drop extending schema failed'
    }
    //debugger;
...
async function createContainedSchemaPositive() {    //user define is default = false
...
async function updateContainedSchema() {    //user define is default = false
...
async function insertDataToContainedTableNegative() {
    ...
    if (
        ...
        logcash.insertDataToContainedTableNegativeError =
            'Insert data to contained table will fail'
    }
...
async function getDataFromContainedTableNegative() {
    ...
    if (
        ...
        logcash.getDataFromContainedTableNegativeError =
            'Get data from contained table will fail'
    }
...
async function dropExtendingContainedTable() {
    ...
    if (res.Ok) {
        ...
        logcash.dropExtendingContainedTableError =
            'Drop extending contained schema failed'
    }
    //debugger;
    await createScheme();
}
...
async function createScheme() {    //user define is default = false
...
async function updateScheme() {    //user define is default = false
    ...
    //debugger;
    if (
        logcash.updateScheme.Name == logcash.createScheme.Name &&
        logcash.updateScheme.Hidden == true
    ) {
        logcash.updateSchemeStatus = true;
    } else {
        logcash.updateSchemeStatus = false;
        logcash.updateSchemeErrorMessage =
...
async function updateSchemeHiddenFalse() {    //user define is default = false
...
async function getDataTableOrderByKey() {
    logcash.getDataTableOrderByKey = await generalService
        ...
        .then((res) => res.Body);
        const schemaModDate = new Date(logcash.updateSchemeHiddenFalse.ModificationDateTime); const millisecondsschemModification = schemaModDate. getTime()
        const objectModeDate1 = new Date(logcash.getDataTableOrderByKey[0].ModificationDateTime); const objectModification1 = objectModeDate1. getTime()
        const objectModeDate2 = new Date(logcash.getDataTableOrderByKey[1].ModificationDateTime); const objectModification2 = objectModeDate2. getTime()
    //debugger;
    if (
