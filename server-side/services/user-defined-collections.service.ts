...
export class UDCService {
    ...
    async createUDCWithFields(collecitonName: string, udcFields: UdcField[], desc?: string) {
        ...
        const udcCreateResponse = await this.generalService.fetchStatus('/addons/api/122c0e9d-c240-4865-b446-f37ece866c22/api/create', {
            method: 'POST',
        ...
        for (let index = 0; index < udcFields.length; index++) {
            ...
            udcUpsertItemResponse.push(
                await this.generalService.fetchStatus(`/addons/api/122c0e9d-c240-4865-b446-f37ece866c22/api/create?collection_name=${collecitonName}`, {
                    method: 'POST',
            ...
            if (udcUpsertItemResponse[udcUpsertItemResponse.length - 1].Ok !== true) {
                ...
                udcUpsertItemResponse[udcUpsertItemResponse.length - 1]["Fail"] = udcUpsertItemResponse[udcUpsertItemResponse.length - 1].Body.fault.faultstring;
            ...
            if (udcUpsertItemResponse[udcUpsertItemResponse.length - 1].Status !== 200) {
                console.log(`UDC Returned Error Status: ${udcUpsertItemResponse[udcUpsertItemResponse.length - 1].Status}`);
                udcUpsertItemResponse[udcUpsertItemResponse.length - 1]["Fail"] = udcUpsertItemResponse[udcUpsertItemResponse.length - 1].Body.fault.faultstring;
                return udcUpsertItemResponse[udcUpsertItemResponse.length - 1];
            }
            if (udcUpsertItemResponse[udcUpsertItemResponse.length - 1].Body[field.Name] !== field.Value) {
                console.log(`UDC Returned Wrong Value: ${udcUpsertItemResponse[udcUpsertItemResponse.length - 1].Body[field.Name]} instaed of: ${field.Value}`);
                udcUpsertItemResponse[udcUpsertItemResponse.length - 1]["Fail"] = udcUpsertItemResponse[udcUpsertItemResponse.length - 1].Body.fault.faultstring;
                return udcUpsertItemResponse[udcUpsertItemResponse.length - 1];
            }
        }
        //4. test all items are upserted
        const udcWithFieldGetResponse = await this.generalService.fetchStatus(`/user_defined_collections/schemes/${collecitonName}`);
